...
local n_salient_vars = 20 + opts.uniform_salient_vars

local n_noise_vars = n_gen_inputs - n_salient_vars

assert(n_salient_vars >= 20 and n_salient_vars < n_gen_inputs,
...
local dist = pdist.Hybrid()
  ...
  :add(pdist.Gaussian{
    n = n_salient_vars - 20,
    mean = torch.CudaTensor(n_salient_vars - 20):fill(0),
    stddev = torch.CudaTensor(n_salient_vars - 20):fill(1),
    fixed_stddev = true
  })
...
for epoch = 1, n_epochs do
  ...
  for col = 1, 10 do
    ...
    for row = 1, rows do
      ...
      col_tensor[{row, {21, 21}}]:fill((col - 5.5) / 2.25)
    end
  end
  ...
  for col = 1, 10 do
    ...
    for row = 1, rows do
      ...
      col_tensor[{row, {22, 22}}]:fill((col - 5.5) / 2.25)
    end
  end
