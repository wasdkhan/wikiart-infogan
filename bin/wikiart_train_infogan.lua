...
local WikiArtDataset = require('WikiArtDataset')
...
Trains an InfoGAN network
  ...
  --rng-seed (default 5423) Seed for random number generation
  --gen-inputs (default 104) Number of inputs to the generator network
...
local n_salient_vars = 40 + opts.uniform_salient_vars

local n_noise_vars = n_gen_inputs - n_salient_vars

assert(n_salient_vars >= 40 and n_salient_vars < n_gen_inputs,
...
local train_data = WikiArtDataset.new('data/wikiart/wikiart64-train.t7')
...
for epoch = 1, n_epochs do
  ...
  for iteration = 1, n_updates_per_epoch do
    ...
    --input = sample.input:narrow(3, 3, 62):narrow(4, 3, 62)
    input = sample.input
  ...
  for col = 1, 10 do
    ...
    local category = col + 10
    for row = 1, rows do
      -- Vary c2 across columns
  ...
  for col = 1, 10 do
    ...
    local category = col + 20
    for row = 1, rows do
      -- Vary c3 across columns
      col_tensor[{row, {21, 30}}]:zero()
      col_tensor[{row, category}] = 1
    end
  end
  local images_varying_c3 = tile_images(generator:forward(gen_input):float(), 5, 10)

  for col = 1, 10 do
    local col_tensor = gen_input_view:select(2, col)
    col_tensor:copy(constant_noise)

    local category = col + 30
    for row = 1, rows do
      -- Vary c4 across columns
      col_tensor[{row, {31, 40}}]:zero()
      col_tensor[{row, category}] = 1
    end
  end
  local images_varying_c4 = tile_images(generator:forward(gen_input):float(), 5, 10)
  ...
  for col = 1, 10 do
    ...
    for row = 1, rows do
      -- Vary c5 from -2 to 2 across columns
      col_tensor[{row, {41, 41}}]:fill((col - 5.5) / 2.25)
    end
  end
  local images_varying_c5 = tile_images(generator:forward(gen_input):float(), 5, 10)
  ...
  for col = 1, 10 do
    ...
    for row = 1, rows do
      -- Vary c6 from -2 to 2 across columns
      col_tensor[{row, {42, 42}}]:fill((col - 5.5) / 2.25)
    end
  end
  local images_varying_c6 = tile_images(generator:forward(gen_input):float(), 5, 10)
  generator:training()
  ...
  local model_disc_file = pl.path.join(model_dir, 'infogan_wikiart_disc.t7')
  torch.save(model_disc_file, discriminator)

  generator:clearState()
  local model_gen_file = pl.path.join(model_dir, 'infogan_wikiart_gen.t7')
  ...
  image.save(
    pl.path.join(image_dir, string.format('varying_c3_%04d.png', epoch)),
    images_varying_c3)

  image.save(
    pl.path.join(image_dir, string.format('varying_c4_%04d.png', epoch)),
    images_varying_c4)

  image.save(
    pl.path.join(image_dir, string.format('varying_c5_%04d.png', epoch)),
    images_varying_c5)

  image.save(
    pl.path.join(image_dir, string.format('varying_c6_%04d.png', epoch)),
    images_varying_c6)

  -- Checkpoint the networks every 10 epochs
  if epoch % 10 == 0 then
    pl.file.copy(model_disc_file,
      pl.path.join(model_dir, string.format('infogan_wikiart_disc_%04d.t7', epoch)))
    pl.file.copy(model_gen_file,
      pl.path.join(model_dir, string.format('infogan_wikiart_gen_%04d.t7', epoch)))
  end
end
