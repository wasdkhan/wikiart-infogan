...
local SvhnDataset = require('SvhnDataset')
local svhnmodel_builder = require('svhnmodel_builder')
...
local train_data = SvhnDataset.new('data/svhn/train_32x32.t7')
...
local generator, discriminator_body, discriminator_head, info_head =
  svhnmodel_builder.build_infogan(n_gen_inputs, dist:n_params())
...
for epoch = 1, n_epochs do
  ...
  if epoch % 10 == 0 then
    pl.file.copy(model_disc_file,
      pl.path.join(model_dir, string.format('infogan_svhn_disc_%04d.t7', epoch)))
    pl.file.copy(model_gen_file,
      pl.path.join(model_dir, string.format('infogan_svhn_gen_%04d.t7', epoch)))
  end
end
