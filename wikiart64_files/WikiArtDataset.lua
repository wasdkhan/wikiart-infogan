-- Data loader for WikiArt

local tnt = require('torchnet')
local argcheck = require('argcheck')

local WikiArtDataset = torch.class('WikiArtDataset', {})

WikiArtDataset.__init = argcheck{
  {name = 'self', type = 'WikiArtDataset'},
  {name = 'data_file', type = 'string'},
  call = function(self, data_file)
    local raw_data = torch.load(data_file)
    self.inputs = raw_data.data:float():div(255)
    self.targets = raw_data.label
  end
}

WikiArtDataset.make_iterator = argcheck{
  {name = 'self', type = 'WikiArtDataset'},
